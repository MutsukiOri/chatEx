...
class GroupUserTile extends StatefulWidget {
  const GroupUserTile({Key? key, required this.userId, required this.groupId})
      : super(key: key);
  final String userId;
          ...
          if (snapshot.hasData) {
            ...
            return GestureDetector(
              onTap: () {
                ...
                Navigator.pushReplacementNamed(context, profileUserShow.id,
                    arguments: {"senderUid": userUid, "isMe": false});
              },
              child: ListTile(
                contentPadding:
                ...
                leading: (userImgUrlFetched.trim().isNotEmpty)
                    ? CircleAvatar(
                        ...
                        backgroundImage:
                            CachedNetworkImageProvider(userImgUrlFetched))
                    : CircleAvatar(
                        radius: 25,
                        backgroundColor: Colors.white,
                        child: Icon(
                ...
                trailing: GroupInfoScreen.isInEditMode
                    ? IconButton(
                        onPressed: () async {
                          await DatabaseService.kickGroupUser(
