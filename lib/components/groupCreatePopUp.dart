...
class GroupCreatePopUp extends StatefulWidget {
  const GroupCreatePopUp(
      {Key? key, required this.userName, required this.userUid})
      : super(key: key);
...
class _GroupCreatePopUpState extends State<GroupCreatePopUp> {
  ...
  createGroup() async {
    ...
    final grpId = await DatabaseService.addNewGroup(
        groupName, widget.userName, widget.userUid);
    final url = await CloudStorageService.addGrpImageGetLink(
        "groupProfilePictures/${grpId}_$groupName.jpg",
        GroupCreatePopUp.image!.path);
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                            hintText: "group name",
                            hintStyle:
                                GoogleFonts.poppins(color: Colors.white12),
              ...
              onPressed: () {
                if (!isGrpCreateLoading) {
                  ...
                  showSnackBar(context,
                      "Group creation under progress cannot cancel!", 1300);
                }
              },
            ...
            ElevatedButton(
              onPressed: () async {
                if (groupName.isNotEmpty && GroupCreatePopUp.image != null) {
                  if (isGrpCreateLoading) {
                    showSnackBar(
                        context, "Group creation under progress!", 1300);
                  } else {
                    createGroup();
                  }
                } else {
...
class _PickDpCircularAvatarState extends State<PickDpCircularAvatar> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
        ...
        child: GroupCreatePopUp.image == null
            ? CircleAvatar(
                radius: 50,
